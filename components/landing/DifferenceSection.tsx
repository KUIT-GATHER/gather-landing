...
export function DifferenceSection() {
  return (
    <section className="relative z-0 py-20 xl:h-[696px] xl:py-0" data-motion-section="difference">
      <div className="relative mx-auto flex w-[calc(100%-40px)] max-w-[1232px] flex-col gap-14 xl:block xl:h-full">
        ...
        <div className="min-w-0 xl:absolute xl:left-0 xl:top-[143px] xl:h-[370px] xl:w-[610px]">
          <div className="flex flex-col gap-2 xl:block xl:h-[226px]">
            ...
            <div className="pt-3 xl:mt-2 xl:h-[112px] xl:pt-5">
              <h2 className="text-3xl font-bold leading-[1.3] text-[#0a0a0a] sm:text-4xl sm:leading-[46px]" data-motion="difference-copy">공고를 보여주는 것에서<span className="hidden xl:inline"><br /></span> 멈추지 않아요</h2>
            </div>
            <div className="xl:mt-2 xl:h-[69px] xl:py-2.5">
              <p className="text-base leading-7 text-[#5e5e5d] sm:text-xl sm:leading-[30px]" data-motion="difference-copy">단순히 봉사 공고를 나열하는 포털이 아닙니다.<span className="hidden xl:inline"><br /></span> Gather는 발견부터 성장까지 이어지는 참여형 커뮤니티예요.</p>
            </div>
          </div>
          <ul className="mt-8 flex flex-col gap-3 text-sm leading-[21px] text-[#101110] xl:gap-0">
            {differenceItems.map((item, index) => <li key={item} className={`flex min-w-0 items-start gap-3 ${index === 0 ? "xl:h-[22px]" : "xl:h-[30px] xl:pt-2"}`} data-motion="difference-item"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#e2f8ee] font-bold text-[11px] leading-[16.5px] text-brand">✓</span><span className="min-w-0">{item}</span></li>)}
          </ul>
        </div>
